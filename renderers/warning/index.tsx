...
const WarningOutput = ({ data, style, classNames, config }: ErrRendererProps) => {
  if (!data) return '';
  ...
  if (!message) return '';
  return <Warning title={ parse(title) } message={ parse(message) } customStyle={ style } config={ config } classNames={ classNames } />;
};
