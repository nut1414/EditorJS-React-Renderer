...
const HeaderOutput = ({ data, style, classNames, config }: ErrRendererProps) => {
  ...
  if (typeof data === 'object' && data.level && typeof data.level === 'number') {
    switch (data.level) {
      case 1: return <h1 style={ h1Style } className={ classNames as any }>{ parse(content) }</h1>;
      case 2: return <h2 style={ h2Style } className={ classNames as any }>{ parse(content) }</h2>;
      case 3: return <h3 style={ h3Style } className={ classNames as any }>{ parse(content) }</h3>;
      case 4: return <h4 style={ h4Style } className={ classNames as any }>{ parse(content) }</h4>;
      case 5: return <h5 style={ h5Style } className={ classNames as any }>{ parse(content) }</h5>;
      case 6: return <h6 style={ h6Style } className={ classNames as any }>{ parse(content) }</h6>;
      default: return <h4 style={ h4Style } className={ classNames as any }>{ parse(content) }</h4>;
    }
  }

  return <h4 style={ h4Style } className={ classNames as any  }>{ parse(content) }</h4>;
};
